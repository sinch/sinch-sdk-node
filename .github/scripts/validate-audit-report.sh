...
jq '.vulnerabilities |= map(select(.data.resolution.path | type == "string" and (startswith("lerna") or startswith("jest") or startswith("@types/jest") or startswith("babel-jest") or startswith("@typescript-eslint") or startswith("@cucumber") or contains("@nestjs/")) | not))' audit-report.json > audit-report-filtered.json

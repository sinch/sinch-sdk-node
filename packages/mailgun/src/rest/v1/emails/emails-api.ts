...
} from '../../../models/v1/emails/response/get-stored-email-response/get-stored-email-response';
import {
...
import { SendEmailResponseFromApi } from '../../../models/v1/emails/response/send-email-response/send-email-response';
import { GenericResponseFromApi } from '../../../models/v1/response/generic-response/generic-response';
import { MailgunStorageHostname, RequestBody, SinchClientParameters } from '@sinch/sdk-client';
import { MailgunDomainApi } from '../mailgun-domain-api';
import { transformSendEmailRequestIntoApiRequestBody } from '../../../models/v1/emails/request/send-email-request/send-email-request.transform';
import { transformSendMimeEmailRequestIntoApiRequestBody } from '../../../models/v1/emails/request/send-mime-email-request/send-mime-email-request.transform';
import { transformGetStoredEmailResponseIntoClientResponse } from '../../../models/v1/emails/response/get-stored-email-response/get-stored-email-response.transform';
import { transformSendingQueuesStatusResponseIntoClientResponse } from '../../../models/v1/emails/response/sending-queues-status-response/sending-queues-status-response.transform';
import { transformSendEmailResponseIntoClientResponse } from '../../../models/v1/emails/response/send-email-response/send-email-response.transform';
...
export class EmailsApi extends MailgunDomainApi {
  ...
  public async getStoredEmail(domainName: string, storageKey: string): Promise<GetStoredEmailResponse> {
    ...
    const apiResponse = await this.client.processCall<GetStoredEmailResponseFromApi>({
      ...
      operationId: 'getStoredEmail',
    });

    return transformGetStoredEmailResponseIntoClientResponse(apiResponse);
  }
  ...
  public async sendEmail(domainName: string, request: SendEmailRequest): Promise<SendEmailResponse> {
    ...
    const body = transformSendEmailRequestIntoApiRequestBody(request);
    const headers: { [key: string]: string | undefined } = {
  ...
  public async sendMimeEmail(domainName: string, request: SendMimeEmailRequest): Promise<SendEmailResponse> {
    ...
    const body = transformSendMimeEmailRequestIntoApiRequestBody(request);
    const headers: { [key: string]: string | undefined } = {
      'Content-Type': `multipart/form-data; charset=utf-8; boundary=${body.getBoundary()}`,
      Accept: 'application/json',
    };
